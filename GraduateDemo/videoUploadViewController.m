...
@interface videoUploadViewController ()<NSURLConnectionDelegate>{
    UIView *mediaView;
...
@implementation videoUploadViewController{
    ...
    #define URL            @"http://10.102.14.107:8000/Upload_Video.php"  // change this URL
...
- (void)viewDidLoad {
    ...
    //NSData *videoData = [NSData dataWithContentsOfFile:self.meidaPathtoUpload];
    NSData *videoData = [NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@", @"file:", self.meidaPathtoUpload]]];
    
    NSLog(@"+++++:%@", self.meidaPathtoUpload);
    pngData = videoData;
    NSLog(@"-------%@",videoData);
    
    //[self initPB];
    //[self videoPlay:self.meidaPathtoUpload];
}
...
- (void)videoPlay:(NSString *)videoPath{
    ...
    CGFloat viewWidth = self.view.frame.size.width - 60;
    playerLayer.frame=CGRectMake(0,0, viewWidth, viewWidth);
    imageView.image = nil;
...
-(BOOL) setParams{
    ...
    if(pngData != nil){
        ...
        [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"uploaded_file\"; filename=\"%@.mov\"\r\n", picName] dataUsingEncoding:NSUTF8StringEncoding]];
